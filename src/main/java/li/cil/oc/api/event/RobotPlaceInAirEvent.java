 ...
 *  CompletableFuture<this> failsAsync(because players can't do this, either).
 * <p/>
...
public class RobotPlaceInAirEvent extends RobotEvent {
    private boolean isAllowed = false;  CompletableFuture<public> RobotPlaceInAirEventAsync(Agent agent) {
        super(agent);
    }

    /**
     * Whether the placement is allowed. Defaults to <tt>false</tt>.
     */  CompletableFuture<boolean> isAllowedAsync() {
        return isAllowed;
    }
     ...
     */  CompletableFuture<Void> setAllowedAsync(boolean value) {
