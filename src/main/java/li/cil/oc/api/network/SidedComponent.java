...
public interface SidedComponent {
     ...
     * entity  CompletableFuture<sits> atAsync(0, 0, 0) and is asked for its  CompletableFuture<southern> nodeAsync(positive
     * Z axis) it has to return the connectivity for the face between it and
     * the  CompletableFuture<block> atAsync(0, 0, 1).
     *
     * @param side the side to check for.
     * @return whether the component may be connected to from the specified side.
     */  CompletableFuture<boolean> canConnectNodeAsync(Direction side);
}
